...
export default async (event, context) => {
  ...
  try {
    const formData = event.headers.get("X-Form-Data")
      ? JSON.parse(event.headers.get("X-Form-Data"))
      : {}
    ...
    return new Response(
      JSON.stringify({ success: true, contactListsResponse }),
      { status: 201 }
    )
  } catch (err) {
    // return new Response(JSON.stringify(err), { status: 400 });
  } finally {
    return new Response(JSON.stringify({ success: true, finally: true }), {
      status: 201
    })
  }
}
